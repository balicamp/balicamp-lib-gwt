...
public class MasterDataUnifiedApprovalMainPanel extends BaseCommonControlComposite {
	...
	@UiField HTMLPanel pnlMainTitle; 
	...
	final  DataChangedEventHandler<?> approveDoneHandler  = new  DataChangedEventHandler(){
		public void handleDataChange(Object data, id.co.sigma.common.client.control.EditorOperation operation) {
			approvalGrid.reload();
		}
	};
	 ...
	 */
	Command reloadListOfApprovalCommand = new Command() {
		@Override
		public void execute() {
	...
	protected void onCariClick (ClickEvent event) {
		approvalGrid.applyFilter(generateQueryFilters());
	}
	 ...
	 * 
	 */
	public void setTitleHTML (String htmlString){
		if ( htmlString== null)
			htmlString=""; 
		this.pnlMainTitle.clear();
		this.pnlMainTitle.getElement().setInnerHTML(htmlString);
