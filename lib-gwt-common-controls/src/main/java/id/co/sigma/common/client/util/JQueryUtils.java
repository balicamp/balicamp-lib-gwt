...
public final class JQueryUtils {
	...
	/**
	 * render tombol dengan icon. icon ini di sarankan icon jquery ui
	 * @param targetElement target element. sebaiknya ini dengan tag<i>button</i>
	 * @param iconCssClass css class dari tombol. sebaiknya ini dari jquery ui thing
	 * @param clickHandler handler click
	 */
	public native void renderIconButton (Element targetElement , String iconCssClass ,   Command clickHandler) /*-{
		var glueMethod =function (){
			try{
				clickHandler.@com.google.gwt.user.client.Command::execute()
			}catch ( e) {
				alert("masalah dalam click button: " + e.message); 
			}
		}
		$wnd.$(targetElement).button ({
			 icons: {
        		primary:iconCssClass
      		},
      		text: false
		}).click(glueMethod); 
	...
	public native void showElement( Element element ) /*-{
		$wnd.$(element).show();
	}-*/;
	...
	public native void hideElement( Element element ) /*-{
		$wnd.$(element).hide();
	}-*/;
	/**
	 * disable element
