...
public class JQLeafMenuWidget<MENUDATA> extends JQBaseMenuWidget<MENUDATA>{
	...
	protected native void deployClickHandler(String methodName )/*-{
		...
		$wnd[methodName]=function (){
			try{
				thisSwap.@id.co.sigma.common.client.jqueryui.menu.JQLeafMenuWidget::propagateClickHandler()();
			}
			catch ( exc){
				$wnd.alert("error deploy click handler di method id.co.sigma.jquery.client.menu.JQLeafMenuWidget<MENUDATA>" + exc.message); 
			}
