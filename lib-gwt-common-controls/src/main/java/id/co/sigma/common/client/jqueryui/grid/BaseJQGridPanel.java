...
import id.co.sigma.common.client.SimpleDataFilter;
import id.co.sigma.common.client.control.DataProcessWorker;
import id.co.sigma.common.client.jqueryui.BaseJqueryWidget;
...
import id.co.sigma.common.client.jqueryui.grid.cols.HaveSubWidgetColumnDefintion;
import id.co.sigma.common.client.jqueryui.grid.cols.ReplaceLabelWorker;
...
import id.co.sigma.common.client.jqueryui.grid.event.GridSelectAllHandler;
...
public abstract class BaseJQGridPanel<DATA> extends SimplePanel {
	 ...
	 * handler sort change
	 **/
	private ISortOrderChangeHandler<DATA> sortOrderChangeHandler ;
	...
	/**
	 * width yang di set ke dalam grid. 
	 * di trigger dengan {@link #setWidth(int)}
	 */
	protected Integer widthSeted  ; 
	...
	/**
	 * task ini di run sebelum grid di render. jadinya kalau di perlukan pekerjaan terhadap column difinition, handle di sini
	 * bisa jadi misal nya : 
	 * <ol>
	 * <li>set column berdasarkan isi dari config</li>
	 * <li>set column label dengan isi dari database etc</li>
	 * </ol>
	 */
	protected void runAdditionalTaskBeforeGridGridRender (BaseColumnDefinition<DATA, ?>[] actualColumnDefs ) {
	...
	protected void renderJQWidgetOnAttachWorker() {
		if ( jqueryRendered)
			return ; 
		putConstructorArgument("resizeStop" , buildOnResizeEndGlueMethod()); 
		if ( multipleSelection)
		...
		runAdditionalTaskBeforeGridGridRender(actualColumnDefinitions);
	...
	protected void propagateSortChange(String rawJsFieldName , int columnIndex , String sortOrder  ){
		if ( sortOrderChangeHandler==null){
			Window.alert("maaf, tidak ada sort handler di definisikan");
			return ;
		}
	...
	protected native JavaScriptObject buildSelectRowHandlerGlueMethod ()/*-{
		...
		return function (rowid){
			...
			swapThis.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::propagateRowSelectEvent(Ljava/lang/String;)(rowid);
		}
	...
	protected native JavaScriptObject buildSortChangeEventHandlerGlueMethod()/*-{
		...
		return function (index, colindex, sortorder){
			try{
				swapThis.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::propagateSortChange(Ljava/lang/String;ILjava/lang/String;)(index, colindex, sortorder);
			}
			catch (e) {
				alert("error dalam proses propagasi sort.error : " + e.message);
			}
		}
	...
	protected native JavaScriptObject buildSelectDoubleClickHandlerGlueMethod ()/*-{
		...
		return function (rowid){
			...
			swapThis.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::propagateRowDoubleClickEvent(Ljava/lang/String;)(rowid);
		}
	}-*/;
	...
	protected native JavaScriptObject buildSelectAllHandlerGlueMethod()/*-{
		...
		return function (rowidArray){
			...
			swapThis.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::propagateSelectAllEventHandler(Ljava/lang/String;)(rowidArray+"");
		}
	...
	/**
	 * plug column width change event
	 */
	protected native JavaScriptObject buildOnResizeEndGlueMethod()/*-{
		var swapThis= this;
		return function ( width,  index ){
			swapThis.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::onColumnResizedRaw(Ljava/lang/String;)( width +":"+ index +"" );
		}
	}-*/;
	...
	/**
	 * generate column tunggal 
	 * @param buttonCellHandler button handler
	 */
	protected StringColumnDefinition<DATA> generateButtonsCell(final CellButtonHandlerWithProcessor<DATA> buttonCellHandler , String defaultLabel ,  String i18Code , int width){
		@SuppressWarnings("unchecked")
		CellButtonHandler<DATA>[] hndlrArr   = (CellButtonHandler<DATA>[]) new CellButtonHandler<?>[]{
			buttonCellHandler
		}; 
		return  generateButtonsCell(hndlrArr , defaultLabel , i18Code , width); 
	}
	...
	protected StringColumnDefinition<DATA> generateButtonsCell(final CellButtonHandler<DATA>[] buttonHandlers , String defaultLabel , String i18Code , int width){
		...
		final BaseJQGridPanel<DATA> swapThis = this ; 
	...
	private native void registerRadioButtonClickHandler( String functionName ) /*-{
		var swapThis = this ;
		var stringPrefix = this.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::radioClickHandlerMethodName ; 
		$wnd[functionName] = function (radioPrefix ) {
			var util = @id.co.sigma.common.client.util.JQueryUtils::getInstance()();
			if ( util==null) {
				alert("util nya null"); 
			}
			util.@id.co.sigma.common.client.util.JQueryUtils::triggerBridgeMethod(Ljava/lang/String;)( stringPrefix + radioPrefix); 
	...
	private native void registerCheckboxClickHandler( String functionName ) /*-{
		var swapThis = this ;
		var stringPrefix = this.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::checkboxClickHandlerMethodName ;
		
		$wnd[functionName] = function (checkPrefix ) {
			var util = @id.co.sigma.common.client.util.JQueryUtils::getInstance()();
			
			util.@id.co.sigma.common.client.util.JQueryUtils::triggerBridgeMethod(Ljava/lang/String;)(stringPrefix + checkPrefix); 
		}
	...
	protected native void putCellButtonClickHandler(String gridReferenceMapName, String methodName )/*-{
		...
		var mthd = function(dataRowId ){
			...
			grid.@id.co.sigma.common.client.jqueryui.grid.BaseJQGridPanel::triggerCellButtonClickHandler(Ljava/lang/String;Ljava/lang/String;)(dataRowId,methodName);
		};
	...
	public void assignSortOrderChangeHandler(
			ISortOrderChangeHandler<DATA> sortOrderChangeHandler) {
	 ...
	 */
	public void dispose() {
		destroyWorker(gridTableElement.getId());
	}
	
	
	
	/**
	 * worker untuk destro grid
	 */
	private native void destroyWorker (String gridId )/*-{
		$wnd.$("#" + gridId).jqGrid("GridDestroy");
	}-*/;
	...
	/**
	 * propagaso column change handler
	 */
	protected void onColumnResizedRaw (String widthIndexSpearatedTitik2  ){
		String[] arr = widthIndexSpearatedTitik2.split("\\:"); 
		
		onColumnResized( Integer.parseInt(  arr[0]) ,  this.actualColumnDefinitions[ Integer.parseInt(arr[1]) ]); 
	}
	
	
	/**
	 * handle pada saat column di selesai di resize
	 * @param width lebar column di akhir
	 * @param changedColumnDef column def yang bergeser
	 */
	protected void onColumnResized (int width , BaseColumnDefinition<DATA, ?> changedColumnDef ){
