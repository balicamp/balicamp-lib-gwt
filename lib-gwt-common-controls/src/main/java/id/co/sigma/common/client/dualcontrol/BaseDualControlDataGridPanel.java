...
public abstract class BaseDualControlDataGridPanel<DATA extends DualControlEnabledData<?, ?>> extends PagedSimpleGridPanel<DATA> implements SimpleSearchFilterHandler{
	...
	private void configureSortChangeHandler () {
	...
	private AsyncCallback swapCallback  = new CommonControlAsyncCallback<PagedResultHolder<DATA>>() {
		@Override
		protected void customFailurehandler(Throwable caught) {
	...
	/**
	 * method ini untuk membuat button pada column actions. kalau diperlukan custom button, override button ini . 
	 */
	@SuppressWarnings("unchecked")
	protected CellButtonHandler<DATA> [] generateActionButtons () {
		GWT.log("[dualcontrol] base column def action buttons");
	...
	/**
	 * lebar default dari action column. override ini kalau anda memerlukan ukuran column yang berbeda
	 */
	...
	protected BaseColumnDefinition<DATA, ?>[] getActualGridColumnDefinitions() {
		...
		actualGrid[1] = this.generateButtonsCell( generateActionButtons(), getActionColumnHeaderLabel()
				, getActionColumnHeaderLabelI18nKey(), getDefaultActionColumnWidth());
