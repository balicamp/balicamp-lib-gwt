...
import id.co.sigma.common.client.dualcontrol.DualControlUtil;
 ...
 * @version $Id
 **/
public abstract class BaseCommonControlEntryPoint implements EntryPoint{
	...
	protected void loadLoginInformationData () {
		UserRPCServiceAsync.Util.getInstance().getCurrentUserLogin(new AsyncCallback<UserDetailDTO>() {
			...
			public void onFailure(Throwable exception) {
				JQueryUtils.getInstance().growlUI("User login info error", "Gagal membaca data login, eror : " + exception.getMessage());
			...
			public void onSuccess(UserDetailDTO userData) {
                ...
                BaseCommonControlComposite.userDetail = userData ;
	...
	protected native void registerAssignPageBaseUrl (String methodName  )/*-{
		...
		$wnd[methodName] = function (baseAppUrl){
			thisSwap.@id.co.sigma.common.client.BaseCommonControlEntryPoint::assignPageBaseUrl(Ljava/lang/String;)(baseAppUrl);
		}
	}-*/;
	/**
	...
	protected native void registerLoadI18AndHideReloaderPanel(String methodName  )/*-{
		...
		$wnd[methodName] = function (langToLoad){
			thisSwap.@id.co.sigma.common.client.BaseCommonControlEntryPoint::loadI18AndHideReloaderPanel(Ljava/lang/String;)(langToLoad);
		}
	}-*/; 
