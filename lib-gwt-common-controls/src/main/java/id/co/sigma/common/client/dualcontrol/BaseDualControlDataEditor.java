...
import id.co.sigma.common.client.ClosePanelCommand;
...
public abstract class BaseDualControlDataEditor<KEY extends Serializable ,  DATA extends DualControlEnabledData<DATA, KEY>> extends BaseEditorPanel<DATA> implements IDualControlEditor<DATA>{
	...
	/**
	 * token yang di pergunakan untuk mengindari double submit. 
	 * token ini di kirimkan bersama dengan data, atau di reqeust terpisah. Ini harus di kirimkan dalam attemp save data(untuk proses approval)
	 */
	private String doubleSubmitAvoidenceToken ; 
	...
	public void openDataByDualControlId (final Long dualControlDataId,final Command commandAfterLoadComplete ) {
		...
		DualControlDataRPCServiceAsync.Util.getInstance().getWaitApprovalDataWithToken(dualControlDataId, new CommonControlAsyncCallback<DataWithToken<CommonDualControlContainerTable>>() {
			...
			public void onSuccess(DataWithToken<CommonDualControlContainerTable> callback) {
				...
				doubleSubmitAvoidenceToken = callback.getToken(); 
	...
	protected void saveForApproval () {
		btnSaveForApproval.setEnabled(false);
		try {
			...
			btnSaveForApproval.setEnabled(true);
			return ; 
		}
	...
	protected void sendRPCRequestFOrSaveForApproval (final DATA editedData) {
		...
		DualControlUtil.getInstance().submitDataForApproval(dualControlEnabledOperation,remark.getValue() ,  editedData, new DoubleSubmitProtectedAsyncCallback<SimpleMasterDataDualControlApprovalResult>() {
			@Override
			public String retrieveDoubleSubmitAvoidenceToken() {
				return doubleSubmitAvoidenceToken;
			}
			@Override
			...
			protected void customFailurehandler(Throwable caught) {
				JQueryUtils.getInstance().unblockEntirePage();
				btnSaveForApproval.setEnabled(true);
				if (caught instanceof SimpleJSONSerializableException){
					SimpleJSONSerializableException swapExc = (SimpleJSONSerializableException) caught ; 
					if ( swapExc.getActualException()!= null)
						caught =  ((SimpleJSONSerializableException)caught).getActualException(); 
				}
				if (caught instanceof InvalidTokenException) {
					JQueryUtils.getInstance().growlUI("Proses 2 kali", "Simpan data hanya bisa di lakukan 1 kali untuk 1 data. mungkin tombol simpan anda tertekan 2 kali. Silakan membuka ulang data anda");
					return ;  
				}
				if ( caught instanceof TokenNotPassedException ){
					JQueryUtils.getInstance().growlUI("token tidak di sediakan" , "Token tidak di kirim dalam penyimpanan. silakan refresh dan di coba lagi.Kalau masalah ini masih terjadi, berarti ada masalah dalam aplikasi. <br/>Silakan laporkan masalah anda ini ke administrator anda. keyword: [double submit],[missing token]" ); 
				}
				
				
				Window.alert(getMessageOnSaveForApprovalFailMessage(caught));
				getCommandOnSaveApprovalFailHandler().execute();
				requestDoubleSubmitToken(null);// request lagi token, agar user bisa save kembali
	...
	protected void onApproveClick() {
		...
		JQueryUtils.getInstance().blockUI(getElement().getId(), getMessageForApproving());
		
		
		DualControlUtil.getInstance().approveData(dualControlEnabledOperation, currentCommonDualControlContainerData, new DoubleSubmitProtectedAsyncCallback<Void>() {
			@Override
			public String retrieveDoubleSubmitAvoidenceToken() {
				return doubleSubmitAvoidenceToken;
			}
			...
			protected void customFailurehandler(Throwable caught) {
				...
				GWT.log("error class : " + caught.getClass().getName());
				JQueryUtils.getInstance().unblockUI(getElement().getId());
				
				if (caught instanceof SimpleJSONSerializableException){
					SimpleJSONSerializableException swapExc = (SimpleJSONSerializableException) caught ; 
					if ( swapExc.getActualException()!= null){
						caught =  swapExc.getActualException();
						GWT.log("actual exception : " + caught.getClass().getName());
						
					}
					else{
						GWT.log("actual exception kosong untuk : " + SimpleJSONSerializableException.class.getName());
					}
				}
	 ...
	 * hide : 
	 * <ol>
	 * <li>remark text box + row sekalian</li>
	 * <li>approval remark sekalian dengan aproval remark</li>
	 * <li>tombol(semua nya)</li>
	 * </ol>
	 * ini di pergunakan kalau editor juga di pergunakan untuk readonly viewer
	 */
	public void hideAllApprovalRelatedItems (boolean hide) {
		String disp = hide ? "none" : "" ; 
		buttonPanel.getElement().getParentElement().getParentElement().getStyle().setProperty("display", disp);
		remark.getElement().getParentElement().getParentElement().getStyle().setProperty("display", disp);
		approvalRemark.getElement().getParentElement().getParentElement().getStyle().setProperty("display", disp);
	}
	 ...
	 * token ini di kirimkan bersama dengan data, atau di reqeust terpisah. Ini harus di kirimkan dalam attemp save data(untuk proses approval)
	 */
	public String getDoubleSubmitAvoidenceToken() {
		return doubleSubmitAvoidenceToken;
	}
	...
	/**
	 * blocker message kalau ada proses reqeust token
	 */
	protected String getMessageForRequestToken () {
		return "loading data.."; 
	}
	
	
	/**
	 * menyetujui data approval
	 */
	protected String getMessageForApproving () {
		return "Menyetujui .."; 
	}
	
	/**
	 * menolak request persetujuan
	 */
	protected String getMessageForRejecting () {
		return "Menyetujui .."; 
	}
	...
	/**
	 * generate double submit avoid token
	 */
	public void requestDoubleSubmitToken (final AsyncCallback<String> callback) {
		JQueryUtils.getInstance().blockUI(getElement().getId(), getMessageForRequestToken());
		GeneralPurposeRPCAsync.Util.getInstance().generateDoubleSubmitAvoidenceToken(new AsyncCallback<String>() {
			@Override
			public void onSuccess(String result) {
				...
				JQueryUtils.getInstance().unblockUI(getElement().getId());
				doubleSubmitAvoidenceToken= result ; 
			}
			@Override
