...
import id.co.sigma.common.client.control.worklist.BaseCommonControlGrid;
...
import id.co.sigma.common.client.lov.NoWidgetLOVCapabledObject;
import id.co.sigma.common.client.util.I18Utilities;
import id.co.sigma.common.client.widget.BaseSimplePopupEditorPanel;
...
/**
 * base class untuk grid dengan client side data container. 
 * jadinya proses editing di lakukan di client( data cuma di simpan di memory - save ke database mempergunakan client side data container)
 * di sarankan untuk membuat base class di level project masing-masing, untuk mengurangi pekerjaan membuat field berulang(kebanyakan masalah i18n)
 * 
 * @author <a href="mailto:gede.sutarsa@gmail.com">Gede Sutarsa</a>
 * @version $Id
 **/
public abstract class BaseClientEditedDataGridPanel<DATA> extends BaseCommonControlGrid<DATA> {
	/**
	 * grid data container
	 **/
	...
	/**
	 * enable erase button atau tidak
	 */
	private boolean enableEraseButton = false ;
	...
	private boolean showEraseButton = true ; 
	private boolean showViewDetailPanel = true ; 
	 ...
	 * enable erase button atau tidak
	 */
	public boolean isEnableEraseButton() {
		return enableEraseButton;
	}
	/**
	 * enable erase button atau tidak
	 */
	public void setEnableEraseButton(boolean enableEraseButton) {
		this.enableEraseButton = enableEraseButton;
	}
	...
	protected void addNewData (){
		try {
			final DATA objBaru = generateNewInstanceData(); 
			GWT.log("finish dengan object. instantiate editor sekarang");
			final BaseSimplePopupEditorPanel<DATA> ed =  getEditorPanel();
			GWT.log("editor ready. menunggu timer");
			new Timer() {
				@Override
				public void run() {
					GWT.log("meluncurkan add new object");
					ed.addAndEditNewData((DATA)objBaru);
				}
			}.schedule(10);
		} catch (Exception e) {
			GWT.log("Gagal proses add, error : " + e.getMessage() , e );
		}
	...
	public void hideDetailButton(boolean hide) {
		this.showViewDetailPanel = !hide ; 
	}
	
	
	/**
	 * show/hide edit button dalam grid 
	 */
	public void hideEditButton(boolean hide) {
		this.showEditButton = !hide ; 
	}
	
	/**
	 * show / hide erase button
	 */
	public void hideEraseButton(boolean hide) {
		this.showEraseButton = !hide ; 
	}
