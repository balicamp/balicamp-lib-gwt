...
import id.co.sigma.common.client.common.LayeredLogicUploadSubmitHandler;
import id.co.sigma.common.client.control.CommonChainedUploadHandler;
		...
		try {
			...
			formUpload.addSubmitHandler(new SubmitHandler() {
				...
				public void onSubmit(SubmitEvent event) {
					JQueryUtils.getInstance().blockUI(getElement().getId(), "Upload data");
				}
			}); 
			...
			formUpload.addSubmitCompleteHandler(new LayeredLogicUploadSubmitHandler() {
				...
				protected void handlerOnUploadSuccessHandler(String uploadedFileKey) {
					JQueryUtils.getInstance().blockUI(getElement().getId() , "Memproses UI");  
			...
			Window.alert("gagal render upload tool. erorr : " + e.getMessage());
		}
