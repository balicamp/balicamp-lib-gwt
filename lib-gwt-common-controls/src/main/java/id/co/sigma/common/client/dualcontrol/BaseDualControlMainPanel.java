...
public abstract class BaseDualControlMainPanel<PK extends Serializable, DATA extends DualControlEnabledData<DATA, PK>> extends BaseCommonControlComposite {
	...
	public BaseDualControlMainPanel(DualControlEditorState dualControlEditorState) {
		...
		if ( getGridHeight()!= null ){
			...
			GWT.log("grid height utnuk class : " + getClass().getName() + ", height tidak di set");
		}
	 ...
	 **/
	protected abstract BaseDualControlDataGridPanel<DATA> instantiateGrid(DataProcessWorker<DATA> editForApprovalRequestHandler  , DataProcessWorker<DATA> viewDetailHandler) ; 
