...
public abstract class ManualJSONSerializeRPCService<RPC extends JSONSerializedRemoteService> {
	...
	public <T>  void submitRPCRequest(final Class<?> serviceInterface, final String methodName , Class<?>[] methodArgumentFQCN , IJSONFriendlyObject<?>[] requestArguemnt , 
		...
		final CommonControlAsyncCallback<T> actualCallback =  rawCallback instanceof CommonControlAsyncCallback? 
				(CommonControlAsyncCallback) rawCallback : 
				new CommonControlAsyncCallback<T>() {
					@Override
		...
		String dataString = "METHOD_TO_INVOKE=" + serviceInterface.getName() + "." 
				...
				"&PARAM_FQCN="; 
		String paramFQCN = "" ; 
		if ( rawCallback instanceof   DoubleSubmitProtectedAsyncCallback){
			DoubleSubmitProtectedAsyncCallback<?> tknCb = (DoubleSubmitProtectedAsyncCallback<?>)rawCallback ; 
			dataString = CommonLibraryConstant.DOUBLE_SUBMIT_AVOIDENCE_PARAMTER_KEY +"=" + tknCb.retrieveDoubleSubmitAvoidenceToken() +"&" +dataString ;  
		}
				...
				public void onResponseReceived(Request request, Response response) {
					if ( response.getStatusCode()==200){
						...
						try {
							...
							GWT.log("gagal memproses raw string :" + rawRequestBody + ", error : " + e.getMessage() , e);
						}	
	...
	protected <T> void handleRequestResponse (String rawString , CommonControlAsyncCallback<T> callback){
