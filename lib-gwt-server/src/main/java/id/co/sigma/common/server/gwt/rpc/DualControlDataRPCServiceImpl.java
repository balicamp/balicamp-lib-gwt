...
public class DualControlDataRPCServiceImpl extends BaseServerRPCService<DualControlDataRPCService> implements DualControlDataRPCService{
	...
	@Autowired
	DualControlRelatedDao dualControlRelatedDao ; 
	...
	public <D extends SystemParamDrivenClass<?, ?>>  AppConfigurationDrivenDetaiResultHolder<D> getSytemConfigurationDrivenData(
		...
		return dualControlDataService.getSytemConfigurationDrivenData(fqcn); 
	}



	@Override
	public PagedResultHolder<CommonDualControlContainerTable> getRejectListData(
			SimpleQueryFilter[] filters, SimpleSortArgument[] sorts,
			int page, int pageSize) {
		return dualControlRelatedDao.getRejectList(filters, sorts, getCurrentUser().getUsername(), page, pageSize);
	}
